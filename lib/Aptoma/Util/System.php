...
class Aptoma_Util_System
{
    ...
    * @param $exitcode int - If argument is present,
    *                      then the return status of the executed command will be written to this variable.
    * @return String output from the command.
    * @throws Aptoma_Util_SystemException if the exec return value wasnt 0.
    *         The exception error code will be set to the return value of the exec command.
    */
    public static function execCommand($command, &$exitcode = null)
    {
        ...
        switch ($exitcode) {
            case 2:
                ...
                break;
            case 126:
                throw new Aptoma_Util_SystemException(
                    'Command found but is not executable (cmd = ' . $command  . ')',
                    $exitcode
                );
                break;
            case 127:
        ...
        if ($exitcode !== 0) {
            throw new Aptoma_Util_SystemException(
                'Command did not exit properly (cmd = ' . $command  . ') (code = ' . $exitcode . ') output: ' . $output,
                $exitcode
            );
        }
